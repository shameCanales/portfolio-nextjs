...
export default function CertificationItem({ item }: CertificationItemProps) {
  ...
  return (
    <div
      className={` p-3 sm:p-6 md:p-3 lg:p-6 xl:p-10 2xl:p-4 3xl:p-8 h-full sm:w-[427px] md:w-auto rounded-xl sm:rounded-2xl md:rounded-xl lg:rounded-2xl mt-5 md:mt-0 sm:mx-auto md:mx-0 bg-[var(--color-card)] ${
        theme === "dark" ? "border-[.5px] border-[var(--color-border)]" : ""
      }`}
    >
      <Image
      ...
      <p className="mt-4 3xl:mt-5 font-bold lg:text-lg 2xl:text-sm 3xl:text-lg leading-normal text-[var(--color-text)]">
        {item.title}
      </p>
      ...
      <Link href={item.certLink}>
        <div className="mt-4 bg-[#FFFFFF] flex items-center px-4 py-2 justify-between sm:w-[174px] md:w-[180px] rounded-sm lg:rounded-md">
          <p className="text-[#1C1C1C] text-sm 2xl:text-xs 3xl:text-sm">
            View Certificate
          </p>
          <Image
      ...
      {/* <p className="mt-6 text-xs lg:text-sm font-medium text-[var(--color-text)]">
        What I learned
      </p>
