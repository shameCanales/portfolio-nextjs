...
export default function GoalCard({ title, description }: CardProps) {
  ...
  return (
    ...
    >
      <h3
        className={`text-sm sm:text-base lg:text-lg 3xl:text-xl tracking-wider font-semibold text-[var(--color-text)] ${outfit.className}`}
      >
        {title}
      </h3>

      <p
        className={`text-xs lg:text-sm 3xl:text-base mt-2 sm:mt-3 lg:mt-2 2xl:mt-3 leading-normal text-[var(--color-text-secondary)] ${outfit.className}`}
      >
        {description}
      </p>
    </div>
